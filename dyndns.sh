...
################ Script Variables ###############################
IPPATH=/home/pi/log/                  # IP address storage file
...
wget -q -U "dyndns.sh wget 1.0" -O $TEMP ip.changeip.com
...
if diff $IPPATH $TMPIP > /dev/null
  then                                # same IP, no update
      ...
      wget -q -U "dyndns.sh wget 1.0" -O $TEMP --http-user=$CIPUSER --http-password=$CIPPASS "https://nic.changeip.com/nic/update?cmd=update&set=$CIPSET"
      if [ $LOGLEVEL -ne 0 ]
